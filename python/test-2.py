...
# DEVICE_ID = "e2a15f05-39cb-48c6-93dc-81e624d07794"
...
#                     # Update device status with GitHub update info
#                     device_data = {
#                         'github_status': 'Update Available',
#                         'updated_at': datetime.datetime.now().isoformat()
#                     }
#                     self.supabase_client.table('devices').update(device_data).eq('id', self.device_id).execute()
#                     self.last_commit_sha = latest_sha
...
SUPABASE_URL = "https://hdodriygzudamnqqbluy.supabase.co"
SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imhkb2RyaXlnenVkYW1ucXFibHV5Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTcyMzcxMTA2OCwiZXhwIjoyMDM5Mjg3MDY4fQ.yNnuOxXhJDSVrcG2X59lEVFdwiKgAOC1kHHL5EMrxsk"
DEVICE_ID = "d2cd2365-7e56-4f31-9046-ce8068ab1975"
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if self.last_commit_sha is None:
                    ...
                    print("New update available!")
                    # Perform the update
                    self.perform_update(owner, repo, self.repo_path, latest_sha)
                else:
                    print("No updates available")
            else:
                print(f"Failed to check for updates: {response.status_code}")
