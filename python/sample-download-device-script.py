...
DEFAULT_CONFIG = {
    ...
    'DEVICE_ID': 'e38bf582-54de-4958-bb78-f8df395a173e',
    'USER_ID': 'b877f0c8-8bd7-49fb-9608-fd47f9da23cf',
    'DEVICE_TYPE': 'Thermostat',
    'DEVICE_TITLE': 'Test #02',
    'INITIAL_VALUE': '0',
...
class DeviceAutomationSystem:
    def __init__(self, device_config: Dict[str, Any]):
        ...
        try:
            # You can either use email/password or a JWT token
            # For now, let's use a special device token that you should set in DEFAULT_CONFIG
            device_token = os.getenv('DEVICE_TOKEN')
            if device_token:
                self.supabase_client.auth.set_session(device_token)
                print("Successfully authenticated with Supabase")
            else:
                print("Warning: No device token provided. Updates may fail due to RLS policies.")
    ...
    def _check_dashboard_connection(self):
        ...
        try:
            ...
            try:
                ...
                update_response = self.supabase_client.table('devices') \
                    .update({
                        'status': 'online',
    ...
    def _notify_dashboard(self, new_value: float):
        ...
        try:
            ...
            if response.data:
                print(f"Device {device_id} updated to {new_value}")
            else:
                print(f"Failed to update device value")
